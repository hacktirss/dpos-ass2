...
LEFT JOIN ( 
            ...
            WHERE v.codigo = '$CODIGO' AND CAST( DATE_FORMAT( v.fecha, '%Y%m%d' ) AS UNSIGNED ) BETWEEN 
                    ( CASE WHEN f.month_day < 16 THEN f.start_month ELSE ( f.middle_month + 1 ) END ) AND
                    ( CASE WHEN f.month_day < 16 THEN f.middle_month ELSE f.end_month END )
        UNION ALL 
            SELECT rm.pesos importe, rm.volumen litros, rm.fecha_venta, rm.cliente, rm.codigo 
            FROM rm 
            JOIN fecha_filter_idx f ON TRUE
            WHERE rm.codigo = '$CODIGO' AND rm.fecha_venta BETWEEN 
                    ( CASE WHEN f.month_day < 16 THEN f.start_month ELSE ( f.middle_month + 1 ) END ) AND
                    ( CASE WHEN f.month_day < 16 THEN f.middle_month ELSE f.end_month END )
