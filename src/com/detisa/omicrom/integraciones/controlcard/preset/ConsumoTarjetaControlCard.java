...
import static com.ass2.volumetrico.puntoventa.preset.Consumo.PRMT_CNS_FP;
...
public class ConsumoTarjetaControlCard extends Consumo {
    ...
    public ConsumoTarjetaControlCard() {
        super();
    }//Constructor
    ...
    public boolean validate() throws DetiPOSFault {
        ...
        ServicioSoap port;
        String account = parameters.NVL(PRMT_ACCOUNT).split("=")[0];
        ...
        try {
            ...
            balance = port.solicitaAutorizacion(account,
            ...
            comando = ComandosVO.parse(manguera, parameters.NVL(PRMT_CNS_TYP), getCant().toPlainString(), balance.getIdTransito() + "|" + account, false);
            comando.setField(ComandosVO.CMD_FIELDS.idtarea, String.valueOf(balance.getIdTransito()));
            LogManager.error(comando);
            posicion.setField(EstadoPosicionesVO.EP_FIELDS.codigo, balance.getIdTransito() + "|" + account);
