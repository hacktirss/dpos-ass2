...
LEFT JOIN ( 
            ...
            WHERE v.codigo = '$CODIGO' AND CAST( DATE_FORMAT( v.fecha, '%Y%m%d' ) AS UNSIGNED ) BETWEEN f.start_month AND f.end_month
        UNION ALL 
            SELECT rm.pesos importe, rm.volumen litros, rm.fecha_venta, rm.cliente, rm.codigo 
            FROM rm 
            JOIN fecha_filter_idx f ON TRUE
            WHERE rm.codigo = '$CODIGO' AND rm.fecha_venta BETWEEN f.start_month AND f.end_month
